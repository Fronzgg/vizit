...
const investorProfileQuestions = [
  { key: 'investmentExperience', title: 'Был ли у вас опыт инвестиций в недвижимость и есть ли сформированный капитал?', options: ['Нет опыта инвестиций в недвижимость и объектов', 'Был опыт инвестиций в недвижимость, но сейчас объектов нет', 'Есть 1-2 объекта', 'Есть 3-5 объектов', 'Есть более 5-ти объектов'] },
  { key: 'savings', title: 'Есть ли у вас накопления для инвестиций?', options: ['Нет накоплений', 'Есть накопления до 500 000 руб.', 'Есть накопления от 500 000 руб до 1 млн.руб.', 'Есть накопления от 1 до 3 млн.руб.', 'Есть накопления 3 млн.руб. +'] },
  { key: 'creditLeverage', title: 'Есть ли у вас возможность использовать кредитное плечо?', options: ['Есть возможность', 'Нет возможности', 'Не знаю, есть ли у меня такая возможность'] },
  { key: 'readiness', title: 'В случае если вы увидите для себя возможность заработать на недвижимости понятным и надежным способом, как скоро вы готовы приобрести объект?', options: ['В ближайший месяц', 'В ближайшие 2-3 месяца', 'В ближайшие 4-6 месяцев', 'В ближайший год', 'Затрудняюсь ответить'] },
] as const

const emptyInvestorProfile = { investmentExperience: '', savings: '', creditLeverage: '', readiness: '' }
...
function BriefTool({ onClose, onSent, calculation }: { onClose: () => void; onSent: () => void; calculation: CalculationData | null }) {
  ...
  const [answers, setAnswers] = useState({ goal: '', budget: '', region: '', name: '', contact: '', ...emptyInvestorProfile })
  ...
  const profileComplete = investorProfileQuestions.every((question) => answers[question.key])
  ...
  const finish = async () => {
    ...
    const message = `Новый запрос с сайта\nИмя: ${answers.name}\nКонтакт: ${answers.contact}\nЗадача: ${answers.goal}\nБюджет: ${answers.budget}\nРегион: ${answers.region}\nОпыт: ${answers.investmentExperience}\nНакопления: ${answers.savings}\nКредитное плечо: ${answers.creditLeverage}\nСрок покупки: ${answers.readiness}${calculationText}`
  ...
  return <ModalShell title="Короткий бриф" eyebrow={`Шаг ${Math.min(step + 1, 5)} из 5`} onClose={onClose}>
    {flying ? <SubmissionFlight onComplete={onSent} /> : step < 3 ? <div className="brief-step"><h3>{questions[step].title}</h3><div className="choice-grid">{questions[step].options.map((option) => <button key={option} onClick={() => { setAnswers({ ...answers, [questions[step].key]: option }); setStep(step + 1) }}>{option}<ArrowUpRight /></button>)}</div></div> : step === 3 ? <div className="brief-step consultation-profile"><h3>Расскажите немного о вашем опыте</h3>{investorProfileQuestions.map((question) => <QuestionChoices key={question.key} title={question.title} value={answers[question.key]} options={[...question.options]} onChange={(value) => setAnswers({ ...answers, [question.key]: value })} />)}<div className="tool-actions"><button className="tool-back" onClick={() => setStep(2)}><ArrowLeft /> Назад</button><button className="tool-primary" disabled={!profileComplete} onClick={() => setStep(4)}>Продолжить <ArrowUpRight /></button></div></div> : <div className="brief-step"><h3>Как с вами связаться?</h3>{calculation && <div className="attached-calculation"><span><Check /> Расчёт прикреплён</span><div><strong>{calculation.budget.toLocaleString('ru-RU')} ₽</strong><small>{calculation.years} лет · {calculation.rate}% в год</small></div><div><strong>{calculation.total.toLocaleString('ru-RU')} ₽</strong><small>расчётная стоимость</small></div></div>}<div className="form-grid"><label><span>Ваше имя</span><input value={answers.name} onChange={(e) => setAnswers({ ...answers, name: e.target.value })} placeholder="Имя" /></label><label><span>Телефон или Telegram</span><input value={answers.contact} onChange={(e) => setAnswers({ ...answers, contact: e.target.value })} placeholder="@username или +7..." /></label></div>{error && <p className="submit-error">{error}</p>}<div className="tool-actions"><button className="tool-back" onClick={() => setStep(3)}><ArrowLeft /> Назад</button><button className="tool-primary" disabled={!answers.name || !answers.contact || sending} onClick={finish}>{sending ? 'Отправляем…' : 'Отправить заявку'} <ArrowUpRight /></button></div></div>}
  </ModalShell>
}
...
function BookingTool({ onClose, onSent }: { onClose: () => void; onSent: () => void }) {
  ...
  const [answers, setAnswers] = useState({ name: '', phone: '', preferredContact: '', ...emptyInvestorProfile })
  const [sending, setSending] = useState(false)
  const [flying, setFlying] = useState(false)
  const [error, setError] = useState('')
  const profileComplete = investorProfileQuestions.every((question) => answers[question.key])
