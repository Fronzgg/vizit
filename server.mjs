...
const storePath = path.join(dataDir, 'site-data.enc.json')
...
async function updateStore(updater) {
  writeQueue = writeQueue.then(async () => { const store = await readStore(); await updater(store); await writeStore(store); return store })
  return writeQueue
}
...
app.post('/api/bookings', async (req, res) => {
  const booking = { id: crypto.randomUUID(), at: new Date().toISOString(), date: cleanText(req.body.date, 20), time: cleanText(req.body.time, 10), contact: cleanText(req.body.contact, 120), status: 'new' }
  if (!booking.date || !booking.time || !booking.contact) return res.status(400).json({ error: 'missing_fields' })
